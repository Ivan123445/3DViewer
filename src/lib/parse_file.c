...
static status_t get_col_points_and_surfaces(gchar *filename,
                                            unsigned int *col_point,
                                            unsigned int *col_surfaces) {
  status_t status = OK;
  *col_point = 0;
  *col_surfaces = 0;
...
static status_t scan_surface(gchar *str, surface_t *surface) {
  ...
  for (int point_num = 0; point_num < surface->count_points; ++point_num) {
    surface->points[point_num] =
        (unsigned short)strtod(str, &str) - 1;  // since the numbering is from 1
  }
...
status_t parse_file(gchar *filename, obj_data_t *obj_data) {
  status_t status = get_col_points_and_surfaces(
      filename, &obj_data->count_points, &obj_data->count_surfaces);
