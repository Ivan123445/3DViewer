...
static char *get_recording_file(gchar *recording_folder) {
    ...
    sprintf(recording_file, "%s/"SAVE_IMAGE_NAME"%d_%d_%d", recording_folder, local->tm_hour, local->tm_min, local->tm_sec);
...
void save_image(GtkWidget *button, GtkWidget *folder_chooser) {
    ...
    char *recording_file = get_recording_file(recording_folder);
    printf("%s\n", recording_file);
    char buf;
